...
def _run_vep_task(file_path, config):
    ...
    try:
        ...
        full_command_string = f"module load VEP/110-GCC-10.2.0 && {vep_command_str}"
...
def step5_calculate_nuclear_coverage(config):
    ...
    output_path = os.path.join(config['final_output_dir'], f"nuc_median_cov_all_batch_{config['batch_name']}.txt")
...
def step6_calculate_mtcn(config, nuc_cov_path):
    ...
    if not os.path.exists(nuc_cov_path) or os.path.getsize(nuc_cov_path) == 0:
        ...
        output_path = os.path.join(config['final_output_dir'], f"mtDNA_CN_batch_{config['batch_name']}.txt")
...
def main():
    batch_num = "4"
    base_project_dir = "/gpfs/gibbs/project/lake_nicole/njl27"
    db_archive_dir = f"/home/njl27/project/past_student_archive/2024/ashton_b/agb66/project/annotation_databases/"
    output_base_dir = "/home/lt692/palmer_scratch/vcf_annotation"

    config = {
        "batch_name": batch_num,
        "num_workers": 50,
        
        # --- MODIFICATION START: Added new required paths ---
        # This TSV file should contain one column with the full path to each CRAM file
        "samples_tsv_path": f"{base_project_dir}/mt_calling/mito_calling_pipeline/samples_file_four.tsv", # Please verify this path
        # This is the reference genome used for alignment
        "reference_genome_path": "/path/to/your/reference.fasta", # IMPORTANT: THIS PATH MUST BE CORRECT
        # --- MODIFICATION END ---
        
        "raw_vcf_dir": f"/home/lt692/palmer_scratch/vcf_annotation/test_vcf/",
        "haplocheck_dir": f"/home/lt692/palmer_scratch/vcf_annotation/test_vcf/",
        "wgs_metrics_dir": f"/home/lt692/palmer_scratch/vcf_annotation/test_vcf/",
        "mt_coverage_dir": f"/home/lt692/palmer_scratch/vcf_annotation/test_vcf/",
        "vep_cache_dir": "/home/njl27/",

        # Output directories
        "vep_vcf_dir": f"{output_base_dir}/vep_vcf/batch{batch_num}/",
        "metadata_dir": f"{output_base_dir}/haplogroup_mtCN/batch{batch_num}/",
        "final_output_dir": f"{output_base_dir}/final_outputs_batch{batch_num}/",

        # Input metadata files
        "fullhaplogroups": f"{output_base_dir}/haplogroup_mtCN/batch{batch_num}/haplogroup_full.txt",
        "contamination": f"{output_base_dir}/haplogroup_mtCN/batch{batch_num}/contamination.txt",
        "master_pedigree_file": f"/home/lt692/palmer_scratch/vcf_annotation/combine_metadata_test.txt",
        
        "gnomadcache": f"{db_archive_dir}/gnomad.genomes.v3.1.sites.chrM.reduced_annotations.tsv",
        "clinvarcache": f"{db_archive_dir}/clinvar_chrMT_02122024.txt",
        "mitomap_polycache": f"{db_archive_dir}/MITOMAP_polymorphisms_02122024.txt",
        "mitomap_diseasecache": f"{db_archive_dir}/MITOMAP_disease_02122024.txt",
        "helixcache": f"{db_archive_dir}/HelixMTdb_20200327.tsv",
        "haplogroup_varcache": f"{db_archive_dir}/rCRS-centered_vars_with_haplo_final_update.txt",
        "mitimpactcache": f"{db_archive_dir}/MitImpact_db_3.1.2.txt",
        "mitotipcache": f"{db_archive_dir}/mitotip_scores.txt",
        "hmtvarcache": f"{db_archive_dir}/hmtvar_annotations.txt",
    }
